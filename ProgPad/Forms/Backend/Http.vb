...
Public Class Http
    ...
    Private LineFeed As Object = Environment.NewLine
...
#Region "Properties"
    ...
    Private _UserAgent As String = "Mozilla/5.0 (Windows NT 6.1; rv:9.0.1) Gecko/20100101 Firefox/9.0.1"
...
request:
        Try
            ...
            Dim _FullPath As String = IIf(Uri.Contains("/"), Uri.Substring(0, Uri.LastIndexOf("/")), Uri)
            ...
            With Request
                ...
                '.Proxy = New WebProxy("219.235.110.13", 1080)
                ...
                If Me.AutoRedirect Then
                    Select Case hr.WebResponse.StatusCode
                        Case HttpStatusCode.Found, HttpStatusCode.Redirect, HttpStatusCode.Moved, HttpStatusCode.MovedPermanently, HttpStatusCode.RedirectMethod
                            ...
                            Catch ex As Exception
                                Uri = _FullPath & IIf(Uri.StartsWith("/"), Uri, "/" & Uri)
                            End Try
...
getHtml:
        ...
        Return hr
    End Function
    Public Function GetUploadResponse(ByVal Uri As String, Optional ByVal PostData() As Byte = Nothing, Optional ByVal Headers As NameValueCollection = Nothing) As HttpResponse
        Dim hr As New HttpResponse
        Dim Response As HttpWebResponse = Nothing
                    ...
                    Select Case hr.WebResponse.StatusCode
                        Case HttpStatusCode.Found, HttpStatusCode.Redirect, HttpStatusCode.Moved, HttpStatusCode.MovedPermanently, HttpStatusCode.RedirectMethod
                            ...
                            Try
                                Dim u As New Uri(Uri)
                            Catch ex As Exception
                                ' Uri = IIf(Request.RequestUri.AbsoluteUri.StartsWith("https"), "https://", "http://") & Request.RequestUri.Host & IIf(Uri.StartsWith("/"), Uri, "/" & Uri)
                                Uri = _FullPath & IIf(Uri.StartsWith("/"), Uri, "/" & Uri)
                            End Try
                            Dim b As Boolean = True
                            For Each r As String In RedirectBlacklist
...
#Region "Private Methods"
    ...
    Private Function FindCookie(ByVal c As HttpCookie) As Boolean ' Search predicate
